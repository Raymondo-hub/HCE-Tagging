...
resource "azurerm_policy_definition" "enforceTag" {
  count = length(var.mandatory_tag_keys)

name = "enforcetag-definition"
policy_type = "Custom"
mode = "All"
display_name = "enforcetag-definition${var.mandatory_tag_keys[count.index]} when creating vitrual machines "
